meta.name = "Hundun Trainer"
meta.version = "1.0"
meta.description =
"Practise the Hundun. Offers a variety of items to start with."
meta.author = "Jools"

local WORLD_SUNKEN_CITY = 7
local HUNDUN_LEVEL = 4
...
set_callback(function()
    if state.world ~= WORLD_SUNKEN_CITY or state.theme ~= THEME.HUNDUN or state.level ~= HUNDUN_LEVEL then
        return
    end
...
set_callback(function()
    ...
    if state.loading == 1 and state.screen_next == ON.LEVEL
        ...
        state.level_next = HUNDUN_LEVEL
        state.theme_next = THEME.HUNDUN

        -- for instant restart
        state.world_start = WORLD_SUNKEN_CITY
        state.level_start = HUNDUN_LEVEL
        state.theme_start = THEME.HUNDUN
    end
end, ON.LOADING)
